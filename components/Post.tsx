...
function Post({ post, modalPost=false }: Data) {
  ...
  text?.length > n ? text.substr(0, n - 1) + "...see more" : text;
    ...
    >
      ...
      <div className="flex justify-evenly items-center dark:border-t border-gray-600/80 mx-2.5 pt-2 text-black/60 dark:text-white/75">
        {modalPost ? (
          ...
          <button
            className={`post-button ${liked && "text-blue-500"}`}
        ...
        {session?.user?.email === post.email ? (
          <button
            className="post-button focus:text-red-400"
            onClick={deletePost}
          >
            <DeleteRoundedIcon />
            <h4>Delete post</h4>
